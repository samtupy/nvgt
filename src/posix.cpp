...
std::cerr << "Aborting: library loader could not load " << lib_name << "\n";
std::exit(1);
} else {
const auto msg = Poco::format("Aborting: library loader could not load %s", std::string(lib_name));
...
libname = lm->l_name;
if (isatty(fileno(stderr))) {
std::cerr << Poco::format("Aborting: library loader could not find symbol %s in library %s\n", std::string(sym_name), libname.empty() ? "NVGT core" : std::string(libname));
std::exit(1);
} else {
const auto msg = Poco::format("Aborting: library loader could not find symbol %s in library %s", sym_name, libname.empty() ? "NVGT core" : std::string(libname));
SDL_ShowSimpleMessageBox(SDL_MESSAGEBOX_ERROR, "Critical error", msg.c_str(), nullptr);
std::exit(1);
}
}
