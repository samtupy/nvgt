...
void RegisterPhysicsCommonFactories(asIScriptEngine* engine) {
    ...
    engine->RegisterObjectBehaviour("physics_sphere_shape", asBEHAVE_FACTORY,
                                    "physics_sphere_shape@ f(float radius)",
                                    asMETHOD(PhysicsCommon, createSphereShape), asCALL_THISCALL_ASGLOBAL, &g_physics);

    engine->RegisterObjectBehaviour("physics_box_shape", asBEHAVE_FACTORY,
                                    "physics_box_shape@ f(const vector&in half_extents)",
                                    asMETHOD(PhysicsCommon, createBoxShape), asCALL_THISCALL_ASGLOBAL, &g_physics);

    engine->RegisterObjectBehaviour("physics_capsule_shape", asBEHAVE_FACTORY,
                                    "physics_capsule_shape@ f(float radius, float height)",
                                    asMETHOD(PhysicsCommon, createCapsuleShape), asCALL_THISCALL_ASGLOBAL, &g_physics);

    engine->RegisterObjectBehaviour("physics_convex_mesh_shape", asBEHAVE_FACTORY,
                                    "physics_convex_mesh_shape@ f(physics_convex_mesh@ convex_mesh, const vector&in scaling = vector(1,1,1))",
                                    asMETHOD(PhysicsCommon, createConvexMeshShape), asCALL_THISCALL_ASGLOBAL, &g_physics);

    engine->RegisterObjectBehaviour("physics_height_field_shape", asBEHAVE_FACTORY,
                                    "physics_height_field_shape@ f(physics_height_field@ height_field, const vector&in scaling = vector(1,1,1))",
                                    asMETHOD(PhysicsCommon, createHeightFieldShape), asCALL_THISCALL_ASGLOBAL, &g_physics);

    // Shape destruction functions
    engine->RegisterGlobalFunction("void physics_sphere_shape_destroy(physics_sphere_shape@ shape)",
                                   asFUNCTION(sphere_shape_destroy), asCALL_CDECL);

    engine->RegisterGlobalFunction("void physics_box_shape_destroy(physics_box_shape@ shape)",
                                   asFUNCTION(box_shape_destroy), asCALL_CDECL);

    engine->RegisterGlobalFunction("void physics_capsule_shape_destroy(physics_capsule_shape@ shape)",
                                   asFUNCTION(capsule_shape_destroy), asCALL_CDECL);

    engine->RegisterGlobalFunction("void physics_convex_mesh_shape_destroy(physics_convex_mesh_shape@ shape)",
                                   asFUNCTION(convex_mesh_shape_destroy), asCALL_CDECL);

    engine->RegisterGlobalFunction("void physics_height_field_shape_destroy(physics_height_field_shape@ shape)",
                                   asFUNCTION(height_field_shape_destroy), asCALL_CDECL);
}

void RegisterShapeConversions(asIScriptEngine* engine) {
    engine->RegisterObjectMethod("physics_sphere_shape",
                                 "physics_collision_shape@ opImplCast()",
                                 asFUNCTION(sphere_to_collision_shape), asCALL_CDECL_OBJFIRST);

    engine->RegisterObjectMethod("physics_box_shape",
                                 "physics_collision_shape@ opImplCast()",
                                 asFUNCTION(box_to_collision_shape), asCALL_CDECL_OBJFIRST);

    engine->RegisterObjectMethod("physics_capsule_shape",
                                 "physics_collision_shape@ opImplCast()",
                                 asFUNCTION(capsule_to_collision_shape), asCALL_CDECL_OBJFIRST);

    engine->RegisterObjectMethod("physics_triangle_shape",
                                 "physics_collision_shape@ opImplCast()",
                                 asFUNCTION(triangle_to_collision_shape), asCALL_CDECL_OBJFIRST);

    engine->RegisterObjectMethod("physics_convex_mesh_shape",
                                 "physics_collision_shape@ opImplCast()",
                                 asFUNCTION(convex_mesh_to_collision_shape), asCALL_CDECL_OBJFIRST);

    engine->RegisterObjectMethod("physics_height_field_shape",
                                 "physics_collision_shape@ opImplCast()",
                                 asFUNCTION(height_field_to_collision_shape), asCALL_CDECL_OBJFIRST);
}
