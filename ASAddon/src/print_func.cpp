...
void Print::PrintTemplate(std::ostream& stream, asIScriptGeneric* generic, int offset) {
	...
	for (int i = offset; i < generic->GetArgCount(); ++i) {
		...
		if (typeId == 0 || (typeId & asTYPEID_OBJHANDLE && ref && *(void**)ref == nullptr))
			continue;
		if (!first)
			stream << " ";
		first = false;
...
void Print::asRegister(asIScriptEngine* engine, bool registerStdStringFormatter) {
	...
	if (registerStdStringFormatter) {
		r = engine->RegisterObjectBehaviour("string", asBEHAVE_CONSTRUCT, "void f(const ?&in, " INS_15 ")", asFUNCTION(PrettyPrinting), asCALL_GENERIC);
		assert(r >= 0);
		r = engine->RegisterObjectMethod("string", "string format(string[]@ elements) const", asFUNCTION(PrettyPrintingArrayF), asCALL_CDECL_OBJFIRST);
		assert(r >= 0);
		r = engine->RegisterObjectMethod("string", "string format(" INS_16 ") const", asFUNCTION(PrettyPrintingF), asCALL_GENERIC);
		assert(r >= 0);
	}
	r = engine->RegisterGlobalFunction("void print(" INS_16 ")", asFUNCTION(PrintFunc), asCALL_GENERIC);
	assert(r == asALREADY_REGISTERED || r >= 0);
	r = engine->RegisterGlobalFunction("void println(" INS_16 ")", asFUNCTION(PrintFuncLn), asCALL_GENERIC);
	assert(r == asALREADY_REGISTERED || r >= 0);
	r = engine->RegisterGlobalFunction("void printf(const string &in format, " INS_16 ")", asFUNCTION(asPrintFormat), asCALL_GENERIC);
	assert(r == asALREADY_REGISTERED || r >= 0);
}
