...
void RegisterScriptFile_Native(asIScriptEngine *engine)
{
    ...
    r = engine->RegisterObjectMethod("file", "bool open(const string &in, const string &in)", asMETHOD(CScriptFile,Open), asCALL_THISCALL); assert( r >= 0 );
    r = engine->RegisterObjectMethod("file", "bool close()", asMETHOD(CScriptFile,Close), asCALL_THISCALL); assert( r >= 0 );
	r = engine->RegisterObjectMethod("file", "int get_size() property", asMETHOD(CScriptFile,GetSize), asCALL_THISCALL); assert( r >= 0 );
	r = engine->RegisterObjectMethod("file", "bool isEndOfFile() const", asMETHOD(CScriptFile,IsEOF), asCALL_THISCALL); assert( r >= 0 );
	r = engine->RegisterObjectMethod("file", "string read(uint = 0)", asMETHOD(CScriptFile,ReadString), asCALL_THISCALL); assert( r >= 0 );
	...
	r = engine->RegisterObjectMethod("file", "int seek(int)", asMETHOD(CScriptFile,SetPos), asCALL_THISCALL); assert( r >= 0 );
...
void RegisterScriptFile_Generic(asIScriptEngine *engine)
{
    ...
    r = engine->RegisterObjectMethod("file", "bool open(const string &in, const string &in)", asFUNCTION(ScriptFile_Open_Generic), asCALL_GENERIC); assert( r >= 0 );
    r = engine->RegisterObjectMethod("file", "bool close()", asFUNCTION(ScriptFile_Close_Generic), asCALL_GENERIC); assert( r >= 0 );
	r = engine->RegisterObjectMethod("file", "int get_size() property", asFUNCTION(ScriptFile_GetSize_Generic), asCALL_GENERIC); assert( r >= 0 );
	r = engine->RegisterObjectMethod("file", "bool isEndOfFile() const", asFUNCTION(ScriptFile_IsEOF_Generic), asCALL_GENERIC); assert( r >= 0 );
	r = engine->RegisterObjectMethod("file", "string read(uint)", asFUNCTION(ScriptFile_ReadString_Generic), asCALL_GENERIC); assert( r >= 0 );
...
#if AS_WRITE_OPS == 1
	r = engine->RegisterObjectMethod("file", "int write(const string &in)", asFUNCTION(ScriptFile_WriteString_Generic), asCALL_GENERIC); assert( r >= 0 );
...
bool CScriptFile::Open(const std::string &filename, const std::string &mode)
{
	...
	string m;
#if AS_WRITE_OPS == 1
	if( mode != "r" && mode != "w" && mode != "a" && mode != "rb" && mode != "wb" && mode != "ab" )
#else
	if( mode != "r" )
#endif
		return false;
...
string CScriptFile::ReadString(unsigned int length)
{
	...
	// If length is 0, read the rest of the file into the string.
	if( length == 0 )
		length = GetSize() - GetPos();
