...
void RegisterScriptMath_Native(asIScriptEngine *engine)
{
	...
	r = engine->RegisterGlobalFunction("float remquo(float a, float b, int& out quo)", asFUNCTIONPR(std::remquo, (float, float, int*), float), asCALL_CDECL); assert(r >= 0);
	...
	r = engine->RegisterGlobalFunction("float frexp(float x, int& out exp)", asFUNCTIONPR(std::frexp, (float, int*), float), asCALL_CDECL); assert(r >= 0);
	...
	r = engine->RegisterGlobalFunction("float modf(float num, float& out iptr)", asFUNCTIONPR(std::modf, (float, float*), float), asCALL_CDECL); assert(r >= 0);
	...
	r = engine->RegisterGlobalFunction("double remquo(double a, double b, int& out quo)", asFUNCTIONPR(std::remquo, (double, double, int*), double), asCALL_CDECL); assert(r >= 0);
	...
	r = engine->RegisterGlobalFunction("double frexp(double x, int& out exp)", asFUNCTIONPR(std::frexp, (double, int*), double), asCALL_CDECL); assert(r >= 0);
	...
	r = engine->RegisterGlobalFunction("double modf(double num, double& out iptr)", asFUNCTIONPR(std::modf, (double, double*), double), asCALL_CDECL); assert(r >= 0);
	...
	r = engine->RegisterEnum("floating_point_classification"); assert(r >= 0);
	r = engine->RegisterEnumValue("floating_point_classification", "FP_NORMAL", FP_NORMAL); assert(r >= 0);
	r = engine->RegisterEnumValue("floating_point_classification", "FP_SUBNORMAL", FP_SUBNORMAL); assert(r >= 0);
