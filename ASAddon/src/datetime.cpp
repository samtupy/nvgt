...
bool CDateTime::set(asUINT year, asUINT month, asUINT day, asUINT hour, asUINT minute, asUINT second)
{
...
void RegisterScriptDateTime(asIScriptEngine *engine)
{
	...
	if(strstr(asGetLibraryOptions(), "AS_MAX_PORTABILITY")==0)
	{
		...
		r = engine->RegisterObjectMethod("datetime", "bool set_date(uint year, uint month, uint day)", asMETHOD(CDateTime, setDate), asCALL_THISCALL); assert(r >= 0);
		r = engine->RegisterObjectMethod("datetime", "bool set_time(uint hour, uint minute, uint second)", asMETHOD(CDateTime, setTime), asCALL_THISCALL); assert(r >= 0);
		r = engine->RegisterObjectMethod("datetime", "bool set(uint year, uint month, uint day, uint hour, uint minute, uint second)", asMETHOD(CDateTime, set), asCALL_THISCALL); assert(r >= 0);
	...
	else
	{
		...
		r = engine->RegisterObjectMethod("datetime", "bool set_date(uint year, uint month, uint day)", WRAP_MFN(CDateTime, setDate), asCALL_GENERIC); assert(r >= 0);
		r = engine->RegisterObjectMethod("datetime", "bool set_time(uint hour, uint minute, uint second)", WRAP_MFN(CDateTime, setTime), asCALL_GENERIC); assert(r >= 0);
