...
if env["PLATFORM"] == "win32":
	env.Append(CCFLAGS = ["/EHsc", "/J", "/MT" if not "windev_debug" in env else "/MTd", "/Z7", "/std:c++20", "/GF", "/Zc:inline", "/Od" if ARGUMENTS.get("debug", "0") == "1" else "/O2", "/bigobj", "/permissive-"])
	...
	env.Append(CXXFLAGS = ["-fms-extensions", "-std=c++20", "-fpermissive", "-O0" if ARGUMENTS.get("debug", "0") == "1" else "-O3", "-g" if ARGUMENTS.get("debug", "0") == "1" else "", "-Wno-narrowing", "-Wno-int-to-pointer-cast", "-Wno-delete-incomplete", "-Wno-unused-result"], LIBS = ["m"])
