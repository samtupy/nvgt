...
vcpkg_path = (
    ...
    / "vcpkg"
    / "bin"
    / ("vcpkg" if sys.platform != "win32" else "vcpkg.exe")
)
oldenv = {}
orig_cwd = Path.cwd()
...
def build_packages_for_android() -> None:
    ...
    if use_official_steam_audio:
        ...
        with zipfile.ZipFile(zip_data) as zf:
            ...
            zf.extract(
                "steamaudio/include/phonon.h", path=str((out_dir / "include").resolve()),
            )
            zf.extract(
    ...
    with Path("droiddev.zip").open("rb") as f, Path("droiddev.zip.blake2b").open("w") as hf:
        h = hashlib.blake2b()
        data = f.read()
        h.update(data)
...
def build_packages_for_linux() -> None:
    ...
    static_packages = [
        ...
        "libogg",
        "opus[*]",
        "sdl3[ibus,x11,wayland,vulkan]",
        "sdl3-ttf[core,svg]",
        "sdl3-image[*]",
        "libvorbis[*]",
        "libarchive[*]",
        "poco[net,netssl,json,util,xml,zip,encodings,mongodb,redis,jwt,prometheus,sevenzip]",
        "freetype[core,bzip2,error-strings,png,subpixel-rendering,zlib]",
    ]
    ...
    if build_arm64_linux:
        if (
            "steam-audio" not in static_packages
            or "steam-audio" not in dynamic_packages
        ):
            sys.exit(
    ...
    if build_arm64_linux:
        ...
        out_dir.mkdir(parents=True, exist_ok=True)
        shutil.copytree(
            vcpkg_path.parent / "installed" / "arm64-linux" / "include",
            out_dir / "include",
            dirs_exist_ok=True,
        )
        shutil.copytree(
            vcpkg_path.parent / "installed" / "arm64-linux" / "lib",
            out_dir / "lib",
            dirs_exist_ok=True,
        )
        shutil.copytree(
            vcpkg_path.parent / "installed" / "arm64-linux" / "debug",
            out_dir / "debug",
            dirs_exist_ok=True,
        )
        shutil.copytree(
            vcpkg_path.parent / "installed" / "arm64-linux-dynamic" / "include",
            out_dir / "include",
            dirs_exist_ok=True,
        )
        shutil.copytree(
            vcpkg_path.parent / "installed" / "arm64-linux-dynamic" / "lib",
            out_dir / "lib",
            dirs_exist_ok=True,
        )
        shutil.copytree(
            vcpkg_path.parent / "installed" / "arm64-linux-dynamic" / "debug",
            out_dir / "debug",
            dirs_exist_ok=True,
        )
    if build_x64_linux:
        ...
        out_dir.mkdir(parents=True, exist_ok=True)
        shutil.copytree(
            vcpkg_path.parent / "installed" / "x64-linux" / "include",
            out_dir / "include",
            dirs_exist_ok=True,
        )
        shutil.copytree(
            vcpkg_path.parent / "installed" / "x64-linux" / "lib",
            out_dir / "lib",
            dirs_exist_ok=True,
        )
        shutil.copytree(
            vcpkg_path.parent / "installed" / "x64-linux" / "debug",
            out_dir / "debug",
            dirs_exist_ok=True,
        )
        shutil.copytree(
            vcpkg_path.parent / "installed" / "x64-linux-dynamic" / "include",
            out_dir / "include",
            dirs_exist_ok=True,
        )
        shutil.copytree(
            vcpkg_path.parent / "installed" / "x64-linux-dynamic" / "lib",
            out_dir / "lib",
            dirs_exist_ok=True,
        )
        shutil.copytree(
            vcpkg_path.parent / "installed" / "x64-linux-dynamic" / "debug",
            out_dir / "debug",
            dirs_exist_ok=True,
        )
        if use_official_steam_audio:
    ...
    if build_arm64_linux:
        out_dir = Path(orig_cwd / "build" / "packages" / "lindev-arm64")
        shutil.make_archive(
            "lindev-arm64", format="zip", root_dir=out_dir, base_dir="lindev",
        )
        with (
