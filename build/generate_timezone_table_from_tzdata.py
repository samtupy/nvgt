#!/usr/bin/env python3
"""
NVGT Timezone Table Generator
Generates C++ timezone map from tzdata binary files
Source: files from Python folder
"""
	...
	for tz_name, offset in timezone_list:
		if tz_name in ['UTC', 'GMT', 'UCT', 'Universal', 'Zulu']:
			...
			if len(parts) >= 2 and parts[0] in regions:
				regions[parts[0]].append((tz_name, offset))
...
def generate_cpp_code(timezone_regions):
	...
	for region_name, timezones in timezone_regions.items():
		...
		for tz_name, offset in timezones:
			entries.append((tz_name, offset))
			total_count += 1

	lines = [
		f"constexpr std::array<std::pair<std::string_view, int>, {total_count}> timezone_data = {{"
	]

	for idx, (tz_name, offset) in enumerate(entries):
