...
public:
    ...
    inline bool advance(Tick delta,
                        size_t max_execute=(size_t)std::numeric_limits<size_t>::max,
                       int level = 0);
    ...
    inline Tick ticks_to_next_event(Tick max = (Tick)std::numeric_limits<Tick>::max,
                                    int level = 0);
        ...
        if (slot_index == 0 && level < MAX_LEVEL) {
            ...
            if (level > 0 || !slots_[level][slot_index].events()) {
                ...
                for (auto event = slot.events(); event != NULL;
                     event = event->next_) {
                    Min = min(Min, event->scheduled_at() - now);
                }
            }
        }
        bool found = false;
