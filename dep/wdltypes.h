...
static int WDL_ASSERT_END()
{
  WDL_ASSERT_INTERNALCNT = 0;
  return 0;
}
...
static void WDL_STATICFUNC_UNUSED wdl_memcpy_le(void *bout, const void *bin, size_t nelem, size_t elemsz)
{
  ...
  if (elemsz > 1)
    wdl_bswap_copy(bout, bin, nelem, elemsz);
  else
#endif
      if (bout != bin)
    memmove(bout, bin, elemsz * nelem);
...
#ifdef __cplusplus
template <class T>
static void wdl_mem_store(void *bout, T v) { memcpy(bout, &v, sizeof(v)); }
template <class T>
static void wdl_mem_store_le(void *bout, T v) { wdl_memcpy_le(bout, &v, 1, sizeof(v)); }
template <class T>
static void wdl_mem_store_be(void *bout, T v) { wdl_memcpy_be(bout, &v, 1, sizeof(v)); }
template <class T>
static T wdl_mem_load(const void *bin)
{
  T v;
  memcpy(&v, bin, sizeof(v));
  return v;
}
template <class T>
static T wdl_mem_load_le(const void *bin)
{
  T v;
  wdl_memcpy_le(&v, bin, 1, sizeof(v));
  return v;
}
template <class T>
static T wdl_mem_load_be(const void *bin)
{
  T v;
  wdl_memcpy_be(&v, bin, 1, sizeof(v));
  return v;
}
#endif
